...
export default {
  ...
  data() {
    return {
      ...
      bannerCrudUrl: {
        // 查找列表数据接口
        selectUrl: '/hart/admin/curriculum/getCurriculumTeacherList'
      },
      searchSlectBannerCrudUrl: {
        selectUrl: '/hart/admin/curriculum/getTeacherKList'
      },
      ...
      bannerTableParams: [
        ...
        { prop: 'nickName', label: '昵称', width: 80 },
  ...
  methods: {
    ...
    bannerDel(index, row) {
      ...
      this.$confirm(tip, '提示', {
        ...
        .then(() => {
          crudList(params, '/hart/admin/curriculum/delCurriculumTeacher')
            .then(res => {
              // 刷新数据
    ...
    postBannerAddForm() {
      ...
      var params = {
        token: this.token,
        teacherIds: this.bannerAddCourse.teacherId,
        // type: this.bannerRadio,
        // picurl: this.upPicUrl,
        curriculumId: this.id
      }
      var couresAddTeacher = '/hart/admin/curriculum/addCurriculumTeacher'
      crudList(params, couresAddTeacher)
        .then(res => {
            this.$message('添加老师成功')
            this.$refs.myBanner_table.fetchData()
