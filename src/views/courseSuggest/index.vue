...
export default {
  ...
  data() {
    return {
      ...
      bannerCrudUrl: {
        // 查找列表数据接口
        selectUrl: '/hart/admin/curriculum/getCurriculumRecdList'
      },
      searchSlectBannerCrudUrl: {
        selectUrl: '/hart/admin/curriculum/getCurriculumKList'
      },
      ...
      bannerTableParams: [
        { prop: 'name', label: '名称'},
        { prop: 'maxCategoryName', label: '类型一', width: 180 },
        { prop: 'minCategoryName', label: '类型二', width: 180},
        // { prop: 'sexName', label: '性别', width: 80 },
  ...
  methods: {
    ...
    bannerDel(index, row) {
      ...
      this.$confirm(tip, '提示', {
        ...
        .then(() => {
          crudList(params, '/hart/admin/curriculum/delCurriculumRecd')
            .then(res => {
              // 刷新数据
    ...
    postBannerAddForm() {
      ...
      var params = {
        token: this.token,
        reccurriculumIds: this.bannerAddCourse.curriculumId,
        // type: this.bannerRadio,
        // picurl: this.upPicUrl,
        curriculumId: this.id
      }
      var couresAddsuggest = '/hart/admin/curriculum/addCurriculumRecd'
      crudList(params, couresAddsuggest)
        .then(res => {
            this.$message('添加相关推荐成功')
            this.$refs.myBanner_table.fetchData()
    ...
    postEdit(row) {
      switch (this.bannerRadio) {
        case '1':
          var params = {
            token: this.token,
            curriculumIds: this.bannerAddCourse.curriculumId,
            curriculumId: this.id,
          }
          break;   
        case '2':
